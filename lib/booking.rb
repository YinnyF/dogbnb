class Booking
  ...
  def self.create(property_id:, renter_id:, owner_id:)
    result = DatabaseConnection.query("INSERT INTO bookings (property_id, renter_id, owner_id) VALUES('#{property_id}', '#{renter_id}', '#{owner_id}') RETURNING id, property_id, renter_id, owner_id;")
    Booking.new(
      id: result[0]['id'],
